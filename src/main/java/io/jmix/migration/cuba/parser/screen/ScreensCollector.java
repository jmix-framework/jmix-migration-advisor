...
import io.jmix.migration.cuba.model.LegacyScreenRegistration;
import io.jmix.migration.cuba.model.UiUnitInfo;
...
public class ScreensCollector {
    ...
    // Single canonical collection of units; the maps below are lookup indexes over it
    protected final List<UiUnitInfo> allUnits;
    protected final Map<String, UiUnitInfo> screensByDescriptors;
    protected final Map<String, UiUnitInfo> screensByControllers;
    protected final Map<String, UiUnitInfo> screensByIds;
     ...
     */
    public Collection<UiUnitInfo> getAllScreens() {
        return List.copyOf(allUnits);
    }

    @Nullable
    public UiUnitInfo getScreenInfoByDescriptor(String xmlDescriptorFileName) {
    ...
    @Nullable
    public UiUnitInfo getScreenInfoByController(String controllerClassName) {
        log.debug("Try to get Screen Info by Controller '{}'", controllerClassName);
        return screensByControllers.get(controllerClassName);
    }

    @Nullable
    public UiUnitInfo getScreenInfoByScreenId(String screenId) {
    ...
    public UiUnitInfo initScreenByXmlDescriptor(String xmlDescriptor) {
        return initScreenInfo(null, xmlDescriptor, null, false);
    }

    public UiUnitInfo initLegacyScreenInfo(String screenId, String xmlDescriptor) {
        return initScreenInfo(screenId, xmlDescriptor, null, true);
    }

    public UiUnitInfo initScreenInfo(String screenId, String xmlDescriptor, String controllerClassName) {
        return initScreenInfo(screenId, xmlDescriptor, controllerClassName, false);
    }

    public UiUnitInfo initScreenInfo(String screenId, String xmlDescriptor, String controllerClassName, boolean legacy) {
        log.debug("Try to init Screen Info with ID = {}, Descriptor ={}, Controller = {}, Legacy = {}",
                screenId, xmlDescriptor, controllerClassName, legacy);

        UiUnitInfo byId = screenId != null ? screensByIds.get(screenId) : null;
        UiUnitInfo byDescriptor = xmlDescriptor != null ? screensByDescriptors.get(xmlDescriptor) : null;
        UiUnitInfo byController = controllerClassName != null ? screensByControllers.get(controllerClassName) : null;

        UiUnitInfo screenInfo = byId != null ? byId : (byDescriptor != null ? byDescriptor : byController);

        if (screenInfo == null) {
            log.debug("Create new Screen Info");

            screenInfo = new UiUnitInfo();
        ...
        // A merged duplicate that lost all its index entries is gone from the canonical collection
        removeIfOrphaned(byDescriptor, screenInfo);
        removeIfOrphaned(byController, screenInfo);

        return screenInfo;
    }

    protected void removeIfOrphaned(@Nullable UiUnitInfo candidate, UiUnitInfo mergeTarget) {
        if (candidate == null || candidate == mergeTarget) {
            return;
        }
        boolean indexed = screensByIds.containsValue(candidate)
                || screensByDescriptors.containsValue(candidate)
                || screensByControllers.containsValue(candidate);
        if (!indexed) {
            allUnits.remove(candidate);
        }
    }
     ...
     */
    protected void mergeScreenInfo(UiUnitInfo target, @Nullable UiUnitInfo source) {
    ...
    public void updateScreenInfo(UiUnitInfo screenInfo) {
        saveScreenInfo(screenInfo);
    }

    protected boolean saveScreenInfo(UiUnitInfo screenInfo) {
